...
class JavaBridge {
  ...
  /*
  private static <K> void appendToMethodMap(HashMap<K, Object> methodMap, K key, MethodHandle mh) {
    Object data = methodMap.get(key);
    if (data == null) {
      methodMap.put(key, mh);
      return;
    }
    if (data instanceof MethodHandle) {  // more than one method handle
      ArrayList<MethodHandle> list = new ArrayList<>();
      list.add((MethodHandle)data);
    ...
    if (data instanceof ArrayList) {
      @SuppressWarnings("unchecked")
        ArrayList<MethodHandle> list = (ArrayList<MethodHandle>)data;
      MethodHandle methodHandle = list.get(0);
      int methodHandleCount = methodHandle.type().parameterCount();
      int mhCount = mh.type().parameterCount();
      if (methodHandleCount == mhCount) {
        list.add(mh);
        return;
      }
      HashMap<Integer, Object> map = new HashMap<>();
      if (list.size() == 1) {
        map.put(methodHandleCount, methodHandle);
      } else {
        map.put(methodHandleCount, list);
      }
      map.put(mhCount, mh);
    ...
    @SuppressWarnings("unchecked")
      HashMap<Integer, Object> map = (HashMap<Integer, Object>)data;
    appendToMethodMap(map, mh.type().parameterCount(), mh);
  }
  
  private static MethodHandle asMH(Method method) {
    MethodHandle mh;
    try {
      mh = MethodHandles.publicLookup().unreflect(method);
    } catch (IllegalAccessException e) {
      //System.err.println("illegal access: skip " + method);
      return null;
    }
    mh = mh.asType(mh.type().generic());
    boolean isStatic = Modifier.isStatic(method.getModifiers());
  ...
  private static MethodHandle createDecisionTreeMH(List<MethodHandle> mhs) {
    MethodHandle mh = mhs.get(0);
    if (mhs.size() == 1) {  
      return mh;
    }
